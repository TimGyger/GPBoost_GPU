...
#pragma omp parallel for schedule(static) reduction(+:d_detmll_d_aux_par, implicit_derivative)
								...
								Log::REInfo("Test %g %g %g %g %g %g %g", neg_likelihood_deriv[ind_ap], d_detmll_d_aux_par, implicit_derivative,
									deriv_information_aux_par.mean(), second_deriv_loc_aux_par.mean(), SigmaI_plus_W_inv_diag.mean(), SigmaI_plus_W_inv_d_mll_d_mode.mean());
							}//end if grad_information_wrt_mode_non_zero_
							else {// grad_information_wrt_mode is zero
								if (use_random_effects_indices_of_data_) {
									if (cg_preconditioner_type_ == "vifdu") {
				...
				for (int j = 0; j < (int)re_comps_ip_cluster_i.size(); ++j) {
					for (int ipar = 0; ipar < re_comps_ip_cluster_i[j]->NumCovPar(); ++ipar) {
						if (estimate_cov_par_index[par_count] > 0) {
							...
							// Derivative of Woodbury matrix
							den_mat_t sigma_woodbury_grad = sigma_ip_grad;
							den_mat_t cross_cov_T_D_plus_WI_inv_diag_cross_cov_grad = (*cross_cov).transpose() * D_plus_WI_inv_diag.asDiagonal() * (*cross_cov_grad);
							sigma_woodbury_grad += cross_cov_T_D_plus_WI_inv_diag_cross_cov_grad + cross_cov_T_D_plus_WI_inv_diag_cross_cov_grad.transpose();
							cross_cov_T_D_plus_WI_inv_diag_cross_cov_grad.resize(0, 0);
